...
@Getter
@Builder
@Schema(description = "훈련일지 댓글 신고 관리자 응답")
public class TrainingLogCommentReportAdminResponse {
    ...
    @Schema(description = "훈련일지 ID")
    private Long entryId;

    @Schema(description = "훈련일지 제목")
    private String entryTitle;

    @Schema(description = "훈련일지 작성자 ID")
    private Long entryAuthorUserId;

    @Schema(description = "훈련일지 작성자 닉네임")
    private String entryAuthorNickname;
    ...
    public static TrainingLogCommentReportAdminResponse from(TrainingLogCommentReport report) {
        return TrainingLogCommentReportAdminResponse.builder()
                .id(report.getId())
                .commentId(report.getComment().getId())
                .entryId(report.getComment().getEntry().getId())
                .entryTitle(report.getComment().getEntry().getTitle())
                .entryAuthorUserId(report.getComment().getEntry().getUser().getId())
                .entryAuthorNickname(report.getComment().getEntry().getUser().getNickname())
