...
- 모든 코드 변경 작업 전에는 `docs/hanes/FAILURE_LOG.md`를 먼저 읽고 변경 대상 도메인 또는 작업 유형과 관련된 과거 실패 사례를 작업 계획에 반영한다.
- 리뷰, 구현, 테스트, 문서화 작업을 시작할 때도 관련 실패 사례가 있는지 `docs/hanes/FAILURE_LOG.md`를 먼저 확인한다.
...
- 리뷰 중 코드 구조, 계약, 테스트, 보안, 운영 측면에서 재발 방지 가치가 있는 이상 구현이나 실패 패턴을 발견하면 `docs/hanes/FAILURE_LOG.md`에 날짜와 내용을 추가할지 검토한다.
