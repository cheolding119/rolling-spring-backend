...
- 오픈매트 작성자 관리 기능은 프론트에 1차 UI가 있다. 현재 상세 화면에서 `참가자 목록 조회`, `참가자 강제 취소`, `모집 상태 수동 변경`을 작성자 전용으로 노출한다.
- 다만 `api-spec.json`에는 작성자 관리 endpoint가 아직 반영되지 않았다. 현재 프론트는 `GET /api/v1/open-mats/{id}/participants`, `DELETE /api/v1/open-mats/{openMatId}/participants/{participantUserId}`, `PATCH /api/v1/open-mats/{id}/status`를 연결 기준으로 준비해 두고, 실서버 계약 확정 전까지는 최종 완료로 간주하지 않는다.
...
- 운영 채널 알림과 대시보드/로그 질의 기준은 아직 남아 있지만, 기본 장애 런북은 `docs/BACKEND_OPERATIONS_RUNBOOK_20260321.md`로 정리됐다. 장애 대응은 여전히 개발자 수동 확인 비중이 높다.
- 신고 기능은 관리자 운영 API까지 확장됐다. 다만 관리자용 `신고 목록/상세/상태 변경`은 동일 대상 기준 누적 신고 3건 이상인 경우만 노출된다.
- 관리자 화면은 신고 처리 워크플로우를 전제로 연결할 수 있고, 노출된 신고에 대해서는 동일 신고 대상 기준 누적 상태도 함께 조회 가능하다.
