...
- [ ] 정원 도달 시 `RECRUITING -> CLOSED` 자동 전환
- [ ] 신청 취소로 여유 발생 시 상태 처리 정책 확정 및 반영
- [ ] `endDateTime` 경과 시 `FINISHED` 자동 전환 (스케줄러 + 조회시 보정)
- [ ] 리스트 정렬/필터 정책을 명세와 일치화
- [ ] 신고 임계치 정책 3건 기준으로 정합성 확정
