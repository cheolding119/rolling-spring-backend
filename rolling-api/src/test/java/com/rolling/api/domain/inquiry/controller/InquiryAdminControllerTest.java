...
class InquiryAdminControllerTest {
    ...
    @Test
    @DisplayName("관리자 문의 목록 조회는 admin accessToken이면 성공한다")
    ...
    @Test
    @DisplayName("관리자 문의 답변 저장은 admin accessToken이면 성공한다")
    void answer_withAdminToken_returnsOk() throws Exception {
        given(jwtTokenProvider.validateToken("admin-token")).willReturn(true);
        given(jwtTokenProvider.getUserIdFromToken("admin-token")).willReturn(1L);
        given(userRepository.existsByIdAndIsWithdrawnFalse(1L)).willReturn(true);
        given(adminAccessConfig.isAdmin(1L)).willReturn(true);
        given(inquiryService.answer(any(), any(), any())).willReturn(answeredResponse(3L));

        mockMvc.perform(patch("/api/v1/admin/inquiries/3/answer")
                        .header("Authorization", "Bearer admin-token")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {
                                  \"answerContent\": \"기기 토큰을 다시 등록해 주세요.\"
                                }
                                """))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.status").value("ANSWERED"))
                .andExpect(jsonPath("$.data.answerContent").value("기기 토큰을 다시 등록해 주세요."));
    }

    @Test
    @DisplayName("관리자 문의 운영 API는 일반 사용자 accessToken이면 403을 반환한다")
    ...
    private InquiryResponse answeredResponse(Long id) {
        return InquiryResponse.builder()
                ...
                .answerContent("기기 토큰을 다시 등록해 주세요.")
                .answeredByUserId(1L)
                .answeredAt(LocalDateTime.of(2026, 3, 20, 12, 0))
    ...
    @Configuration
    @EnableWebMvc
    @Import({InquiryAdminController.class, GlobalExceptionHandler.class, SecurityConfig.class, SpringDataWebConfiguration.class})
