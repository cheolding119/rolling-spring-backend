...
- 오픈매트 생성/수정 request는 선택 필드 `imageUrls`를 지원한다.
- 오픈매트 목록/상세 응답은 `imageUrls: String[]`를 additive 하게 포함한다.
- 생성 request에서 `imageUrls`가 없거나 `null`이면 이미지 없이 저장한다.
- 수정 request에서 `imageUrls`가 없으면 기존 이미지를 유지하고, `[]`면 모두 삭제하며, `null`이면 `VALIDATION_ERROR`다.
