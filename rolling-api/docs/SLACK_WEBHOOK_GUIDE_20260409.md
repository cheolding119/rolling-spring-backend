...
- 5xx 비율이 기준 이상으로 유지되는 경우
  현재 기준: 최근 5분 rate 기준 5xx 비율이 `5% 초과` 상태로 `10분 이상` 유지
- p95 응답 시간이 기준 이상으로 유지되는 경우
  현재 기준: 최근 5분 histogram 기준 p95 응답 시간이 `1.5초 초과` 상태로 `10분 이상` 유지
- tournament crawler가 오랫동안 성공하지 못한 경우
- openMatStatusSync 실패가 최근 발생한 경우
- FCM 실패율이 급증한 경우
  현재 기준: 최근 15분 동안 전체 전송 `10건 이상`일 때 실패율이 `20% 초과` 상태로 `10분 이상` 유지

추가로 이해하면 좋은 점:

- `5xx 비율`은 전체 요청 중 서버 오류 응답의 비율이다.
- `p95 응답 시간`은 요청 100개 중 느린 쪽 5개를 제외한 경계 지점 시간이라고 생각하면 된다.
- `FCM 실패율`은 푸시 전송이 아예 적은 구간에서는 오탐이 나지 않도록 최소 전송 수 `10건` 조건을 같이 둔 상태다.
