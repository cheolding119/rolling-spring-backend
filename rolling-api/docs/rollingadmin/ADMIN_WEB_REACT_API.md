...
- 실제 제재 정책과 제한 모드는 `docs/usersanction/USER_SANCTION_POLICY.md`를 기준으로 맞춘다.
...
- 응답의 `accountStatus`, `suspensionUntil`, `sanctionReasonSummary`는 세션 저장소에 함께 보관해 두고, 운영 계정 제재 상태 진단이 필요할 때 사용한다.
...
- 갱신 성공 시 토큰 교체
- 갱신 성공 시 `accountStatus`, `suspensionUntil`, `sanctionReasonSummary`도 함께 갱신
...
- 운영 계정이 제재 상태인 경우를 대비해 `accountStatus`, `suspensionUntil`, `sanctionReasonSummary`를 로컬 사용자 상태와 같이 유지한다
...
- 심각한 위반도 별도 계정 상태 추가 없이 `무기한 정지`로 운영한다.
...
- `q`: 닉네임, 이메일, 소속, 전화번호, 사용자 ID 검색어
- `status`: `ACTIVE | WARNING | SUSPENDED | WITHDRAWN`
- `page`
- `size`
- `sort`
...
```json
{
  ...
  "suspensionUntil": "2026-04-20T00:00:00",
  "lastSanctionAt": "2026-04-14T13:00:00"
}
```

React 메모:

- `accountStatus`와 `suspensionUntil`로 현재 정지 여부와 만료 예정 시각을 목록 테이블에서 바로 보여준다.
- 장기 정지 preset을 쓴 경우에도 응답 타입은 계속 `SUSPENDED`다.
...
```json
{
  ...
  "accountStatus": "SUSPENDED",
  "suspensionUntil": "2026-04-20T00:00:00",
  "sanctionReasonSummary": "반복적인 욕설",
  "isWithdrawn": false,
  "withdrawalPending": true,
  "withdrawalScheduledAt": "2026-04-17T21:00:00",
  "lastSanctionAt": "2026-04-14T13:00:00"
}
```
...
- 상세 화면에서는 `accountStatus`, `suspensionUntil`, `sanctionReasonSummary`를 상태 카드로 묶어 보여준다.
- 정지 사용자도 탈퇴 예약과 취소가 가능하므로 `withdrawalPending`, `withdrawalScheduledAt`를 항상 함께 노출한다.
...
- `type`은 `WARNING` 또는 `TEMP_SUSPEND`만 허용한다.
- `type=TEMP_SUSPEND`인 경우 `endsAt`이 반드시 필요하다.
- `type=WARNING`인 경우 `endsAt`은 보내면 안 된다.
- `무기한 정지`도 API 타입은 `TEMP_SUSPEND`를 사용하고, `endsAt`은 매우 먼 미래 시각으로 저장한다.
- `type`이 누락되거나 `null`이면 서버에서 `VALIDATION_ERROR`로 처리된다.
- `reason`이 비어 있으면 서버에서 `VALIDATION_ERROR`로 처리된다.
...
- 강한 제재도 별도 제재 타입 추가 없이 `TEMP_SUSPEND` + 매우 긴 `endsAt`으로 처리한다.
- 관리자 화면에서는 이를 `무기한 정지` 또는 `100년 정지` 같은 preset으로 제공할 수 있다.

생성 응답 필드는 `UserSanctionResponse`를 그대로 사용한다.
...
```json
{
  ...
  "createdAt": "2026-04-14T13:00:00",
  "releasedByUserId": 1,
  "releasedAt": "2026-04-16T10:00:00"
}
```

React 메모:

- 관리자 웹의 제재 생성 폼은 `type`, `reason`, `memo`, `endsAt`만 보내면 된다.
- `무기한 정지` preset은 별도 enum을 만들지 말고 `TEMP_SUSPEND` + 매우 먼 미래 `endsAt`로 구성한다.
- 해제 후에는 상세 카드와 제재 이력 목록을 같이 재조회해 `accountStatus`, `suspensionUntil`, `sanctionReasonSummary`를 즉시 갱신한다.
...
이번 백엔드 기준으로 어드민 영역에서 추가되거나 변경된 내용은 아래와 같다.
...
- `suspensionUntil`
- `sanctionReasonSummary`
- `user_sanctions` 이력 테이블

### 13.3 변경된 인증 / 세션 동작

- 로그인 응답에 계정 상태 정보가 포함된다
- 토큰 갱신 응답에 계정 상태 정보가 포함된다
- `TEMP_SUSPEND`는 로그인은 허용하되 제한 모드만 허용한다
- 강한 제재도 별도 로그인 차단 상태가 아니라 `TEMP_SUSPEND` 기반 제한 모드로 처리한다

### 13.4 관리자 웹에서 확인할 포인트

- 사용자 관리 화면은 목록과 상세를 분리해서 본다
- 상세 화면에서 현재 상태와 제재 이력을 함께 본다
- 제한 모드에서는 문의, 도움말, 알림 설정, 차단한 사용자 관리, 탈퇴 요청/취소, 로그아웃이 허용된다
- 사용자 정보 수정과 오픈매트 / 대회 / 신고 생성은 기본 차단한다
- 사용자 상세에서 `withdrawalPending`, `withdrawalScheduledAt`를 함께 보고 정지 중 탈퇴 예약 여부를 확인해야 한다
